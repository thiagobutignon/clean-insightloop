...
export class JWTAuthService {
  ...
  async login(loginDto: LoginUserDTO, context: LoginContext): Promise<AuthResult> {
  ...
  async refreshTokens(refreshToken: string): Promise<TokenPair> {
    try {
      ...
      const isCompromised = await this.tokenBlacklist.isTokenFamilyCompromised(decoded.tokenFamily);
      if (isCompromised) {
...
export class APIKeyService {
  ...
  async verifyAPIKey(keyValue: string): Promise<APIKey | null> {
    ...
    // Verify secret
    const isValid = await this.verifyAPIKeySecret(keySecret, apiKey.hashedSecret);
    if (!isValid) {
      return null;
    }
  ...
  private async verifyAPIKeySecret(secret: string, hash: string): Promise<boolean> {
